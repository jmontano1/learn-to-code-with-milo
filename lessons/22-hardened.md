...
- [ ] **Ledger:** a **link to a green CI run** (your instructor opens it), the red-X-then-fixed proof, and your load-test **script MILO can re-run** — not a pasted number — are in `forge/PROOF.md`.
- [ ] **Built live under watch:** the core security fix was written *live* in a session MILO or your instructor watched. Rehearse the attack surface with MILO first (*"find the door I left open, the input that breaks it, the way to run up my costs"*) and fix what it finds.
- [ ] **Live gate (with your instructor):** on the call your instructor names one security behavior and watches red-to-green in real time — *"make this endpoint reject a blank email and prove it with a real request against your live URL, right now"* — you predicting out loud what will happen before you run it. They watch your real system change, not a screenshot.
